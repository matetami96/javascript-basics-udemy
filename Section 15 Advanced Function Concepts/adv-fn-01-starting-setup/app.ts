...
function addRandom(num1: number) {
...
function addMoreNumbers(num1: number, num2: number) {
	const sum = num1 + num2;
	previousResult = sum;
	return sum;
}
...
// 'tax' in this example
function createTaxCalculator(tax: number) {
...
function powerOf(x: number, n: number): number {
	// if (n === 1) {
	// 	return x;
	// }
...
function getFriendNames(person: Person) {
